...
./kubectl \
  ...
  patch namespace flux-system   -p '{"metadata":{"finalizers":null}}'
  kubectl get ns flux-system -o json | jq '.spec.finalizers = []' | kubectl replace --raw "/api/v1/namespaces/flux-system/finalize" -f -
